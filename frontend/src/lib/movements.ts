...
const APPROACH_HEADING: Record<Approach, number> = {
  NB: 0,
  EB: 90,
  SB: 180,
  WB: 270,
...
const APPROACH_PLACEMENT_BEARING: Record<Approach, number> = {
  NB: 180,
  SB: 0,
  EB: 270,
  WB: 90,
...
function isLeftOnly(lanes: LaneKind[]) {
  return (
    lanes.includes("left") &&
    !lanes.includes("through") &&
    !lanes.includes("right")
  );
}
...
export function defaultMovementPosition(
  ...
  const approachPoint = offset(
    baseLat,
    baseLon,
    APPROACH_PLACEMENT_BEARING[approach],
    30,
  );
...
export function movementIconHtml(
  movement: Movement,
  color: string,
  selected?: boolean,
): string {
